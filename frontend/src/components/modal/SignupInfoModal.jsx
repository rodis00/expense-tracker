...
const SignupInfoModal = ({ signupCounter, version }) => {
  const dispatch = useDispatch();

  const info = [
    "Hey you are already have account :)",
    "Hey you are really have account, go and check it out :)",
    "Dear user, why are you doing this? no need, you are already have account",
  ...
  return (
    <Modal open={version === "signupInfo"}>
      <div className="w-full h-60 flex flex-col justify-center items-center gap-8 text-white rounded-3xl border-2 border-white">
        ...
        />
        <p className="text-lg px-8">{info[signupCounter]}</p>
        <button
